...
# 匹配车牌的正则对象
REGEX = re.compile(
    '^[{}][0-9a-z]{{6}}$'.format('|'.join(data.PROVICE_ABBREVS)), flags=re.IGNORECASE)
...
if __name__ == '__main__':
    ...
    for name in ['川a6882q', '川a6926q', '云uk2wby']:
        test(name)

    print('-' * 50)
    for pair in data.search_all(data.REMOTE_DB, data.TABLE)[0:3]:
        test(pair[0])
